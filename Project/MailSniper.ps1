...
.PARAMETER $OutputCsv
  ...
  Outputs the results of the search to a Xls file.
...
#>
  ...
  (
    ...
    [string[]]$Terms = ("*password*","*creds*","*credentials*","*name*","*time*","*meeting*","*location*","*iban*","*pin*"),
...
'@
  ...
  if ($OutputXls -ne "")
  {
    $filedata = Import-Csv $OutputXls -Header Sender , ReceivedBy , Subject , Body
    $filedata | Export-Csv $OutputXls -NoTypeInformation
    Write-Host -foregroundcolor "yellow" "`r`n[*] Results have been output to $OutputXls"
  }
  #Remove User from impersonation role
  Write-Output "`r`n[*] Removing ApplicationImpersonation role from $ImpersonationAccount."
  Get-ManagementRoleAssignment -RoleAssignee $ImpersonationAccount -Role ApplicationImpersonation -RoleAssigneeType user | Remove-ManagementRoleAssignment -confirm:$false
...
<#
  ...
  .EXAMPLE
    ...
    C:\PS> Invoke-SelfSearch -Mailbox current-user@domain.com -ExchHostname mail.domain.com -OutputXls mails.csv -Remote
...
'@
  ...
  if ($OutputXls -ne "")
  {
    ...
    $PostSearchList | %{ $_.Body = $_.Body -replace "`r`n",'\n' -replace ",",'&#44;'}
    $PostSearchList | Select-Object Sender,ReceivedBy,Subject,Body | Export-Csv $OutputXls -encoding "UTF8"
  ...
  .DESCRIPTION
    ...
    This module will connect to Google using a set of user credentials and search a user's inbox for certain terms.
        ...
        .EXAMPLE
            ...
            PS C:> Invoke-SearchGmail -EmailAddress email@gmail.com -Password Summer2017 -Search search-term -OutputXls out.csv
#>
...
'@ | Out-null
        ...
        $fullresultsarray | Export-Csv -Encoding UTF8 $OutputXls
        Write-Output "[*] Results have been written to $OutputXls."
}
